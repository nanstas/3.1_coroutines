...
import ru.netology.authorid.dto.Author
import ru.netology.authorid.dto.Comment
import ru.netology.authorid.dto.Post
import ru.netology.authorid.dto.PostWithAuthorAndComments
        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostWithAuthorAndComments(post, getAuthor(client, post.authorId), getComments(client, post.id))
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
